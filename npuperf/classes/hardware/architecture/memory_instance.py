...
class MemoryInstance:
    def __init__(self, name: str, size: int, r_bw: int, w_bw: int, r_cost: float, w_cost: float, area: float,
        ...
        :param random_bank_access: whether the memory support random bank access or not. 是否支持随机存储访问
        ...
        :parm min_r_granularity: 最小read 粒度, 默认为read带宽 (bits/cycles)
        :parm min_w_granularity: 最小write 粒度, 默认为write带宽 (bits/cycles)    
        """
        ...
        self.area = area
        self.bank = bank
        self.random_bank_access = random_bank_access    # 是否支持随机存储访问
        self.r_port = r_port
        self.w_port = w_port
    ...
    def __str__(self) -> str:
        return f"MemoryInstance_{self.name}"
