...
class MemoryPort:
    ...
    def __eq__(self, other) -> bool:
        return str(self) == str(other)      # 根据port name区别两个不同的物理port
...
class MemoryLevel:
    ...
    def __init__(self, memory_instance: MemoryInstance, operands: Tuple[str, ...], mem_level_of_operands: Dict[str, int],
        ...
        self.operands = list(operands)          # 把tuple转成list， 例如('R', 'O') -> ['R', 'O']
        self.mem_level_of_operands = mem_level_of_operands  # str 是此mem lv支持的操作数，int是此操作数方向上的级别（从0开始）
        self.served_dimensions_vec = served_dimensions      # e.g.:  served_dimensions={(0, 0, 1, 0), (0, 0, 0, 1)}, 这里传进来的要先check，check之后给 self.served_demensions
        self.dimensions = operational_array.dimensions  # Dimension class的三个参数：idx, name str, size
        ...
        '''
        对于当前内存级别保持的每个操作数, 为其4个潜在的数据移动分配物理内存端口 \\
        ...
        # 如果外部传进来了id，就分配id，否则用内部的__identifier给id
        if id is not None:
    ...
    def __str__(self):
        self.__update_formatted_string()
        return f"{self.name}-{self.operands}"  # 先改成这种简单的格式
    ...
    def calc_unroll_count(self):
        """
        total fanout 和 unroll count:
        这两个数相乘必等于总的PE 个数   \\
        而 fanout 又等于 served dimensions 对应的dim size的乘积
    ...
    def calc_fanout(self):
        """
        扇出    \\
        如果 served dimensions 是 all, 那这个数等于PE 的总个数, 即所有维度大小的乘积 \\
        否则等于对应 served dimension 上那个维度大小的乘积
    ...
    def check_served_dimensions(self):
        """
        如果此MemoryLevel是空集或“all”, 则修改其served_dimensions的函数。\\
        空集表示内存级别没有维度服务于以下级别, 因此扇出1。\\
        'all' 表示MemoryLevel的 served_dimensions 都是维度, 因此在这个级别上只有一个MemoryNode实例。    \\
