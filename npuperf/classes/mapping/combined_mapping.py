...
from npuperf.classes.mapping.temporal.temporal_mapping import TemporalMapping
...
class FourWayDataMoving:
    ...
    def __init__(self, rd_out_to_low, wr_in_by_low, rd_out_to_high, wr_in_by_high):
        ...
        self.info_sum = self.rd_out_to_low + self.wr_in_by_low + self.rd_out_to_high + self.wr_in_by_high

    def update_single_dir_data(self, dir: str, new_value):  # dir 即为四种数据移动行为中的一个
    ...
    def get_total_read_outs_to_above(self, scaling: float = 1):
        ...
        :parm scaling: 作为单次read能量开销 可以计算总的read energy
        """
    ...
    def __add__(self, other):
        # 定义两个 FourWayDataMoving实例之间的加法运算，即对应位置相加
    ...
    def __mul__(self, other):
        # 定义一个 FourWayDataMoving实例和一个系数之间的乘法运算，即对应位置分别乘以系数
    ...
    def _info_sum(self):    # 从这里返回 sum 值是OK的，从 self.info_sum 返回貌似有点问题？
        return (self.rd_out_to_low + self.wr_in_by_low + self.rd_out_to_high + self.wr_in_by_high)
...
class DataMovePattern:
    ...
    def __jsonrepr__(self):
        return {
            "name": self.name,
            "data_elem_move_count": self.data_elem_move_count,
            "data_precision": self.data_precision,
            "req_mem_bw_aver": self.req_mem_bw_aver,
            "req_mem_bw_inst": self.req_mem_bw_inst,
            "data_trans_period": self.data_trans_period,
            "data_trans_period_count": self.data_trans_period_count,
            "data_trans_amount_per_period": self.data_trans_amount_per_period,
            "inst_data_trans_window": self.inst_data_trans_window,
        }
    ...
    def set_data_trans_period(self, rd_out_to_low, wr_in_by_low, rd_out_to_high, wr_in_by_high):
        ...
        """ data_trans_period: 每隔多少个周期, 内存链路需要激活一段时间 """
    ...
    def set_data_trans_period_count(self, rd_out_to_low, wr_in_by_low, rd_out_to_high, wr_in_by_high):
        ...
        """ data_trans_period_count: 要完成所有 for 循环计算, 需要多少个这样的理想周期 """
    ...
    def set_data_trans_amount_per_period(self, rd_out_to_low, wr_in_by_low, rd_out_to_high, wr_in_by_high):
        ...
        """ data_trans_amount_per_period: 每个单周期传输的数据量 """
    ...
    def set_inst_data_trans_window(self, rd_out_to_low, wr_in_by_low, rd_out_to_high, wr_in_by_high):
        ...
        """ inst_data_trans_window: 允许的内存更新窗口, 假设所提供的内存级别为非双缓冲 (因此需要避免数据覆盖问题) """
...
class Mapping:
    ...
    def __jsonrepr__(self):
        return {
            "unit_mem_data_movement": self.unit_mem_data_movement,
            "combined mapping": {
                "total data size 1s1t": self.combined_mapping_dict_1s1t_reform,
                "unroll data size 1s2t": self.combined_mapping_dict_1s2t_reform,
            },
            "patial sum flag": self.psum_flag,
            "data precision dict": self.data_precision_dict,
            "r ir loop list": {
                "r_loop_size_cabl   ": self.r_loop_size_cabl   ,
                "r_loop_size_cabl2  ": self.r_loop_size_cabl2  ,
                "ir_loop_size_cabl  ": self.ir_loop_size_cabl  ,
                "ir_loop_size_cabl2 ": self.ir_loop_size_cabl2 ,
                "O_ir_loop_size_caal": self.O_ir_loop_size_caal,
            },
            "data size": {
                "data_elem_per_level_unrolled": self.data_elem_per_level_unrolled,
                "data_bit_per_level_unrolled ": self.data_bit_per_level_unrolled ,
                "data_elem_per_level": self.data_elem_per_level,
                "data_bit_per_level": self.data_bit_per_level,
            },
            "effective data size": {
                "elem": self.effective_data_elem,
                "bit": self.effective_data_bit,
            },
            "data access": {
                "data access raw": self.data_access_raw,
                "data access raw2": self.data_access_raw2
                },
            "others": {
                "layer node": self.layer_node,
                "operand list": self.operand_list,
                "mem level": self.mem_level,
            }
        }
...
if __name__ == "__main__":
    ...
    cm = Mapping(spatial_mapping_dic, temporal_mapping_dic, layer_node)
    print(f'Total MAC counts: {cm.layer_node.total_MAC_count}')
    
    pprint(cm.combined_mapping_dict_1s1t)
    pprint(cm.combined_mapping_dict_1s2t)
    
    # 上面两个decouple后产生reform 的combined mapping
    pprint(cm.combined_mapping_dict_1s1t_reform)
    pprint(cm.combined_mapping_dict_1s2t_reform)
    
    # data access raw，根据 combined mapping 计算
    print(cm.data_access_raw)
    print(cm.data_access_raw2)
    
    print(cm.unit_mem_data_movement['O'][0].data_elem_move_count)
    print(cm.operand_list)
    print(cm.mem_level)
    # print(cm.__jsonrepr__())
