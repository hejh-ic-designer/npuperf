...
import numpy as np
from npuperf.classes.mapping.combined_mapping import Mapping
...
if TYPE_CHECKING:
    ...
    from npuperf.classes.mapping.combined_mapping import SpatialMapping, TemporalMapping
...
class PortActivity:
    ...
    def __init__(self, real_cycle: int, allowed_cycle: int, period: int, period_count: int,
        ...
        # 这里有用的两个变量：self.SS, self.MUW，这是单个端口的数据
    ...
    def __simplejsonrepr__(self):
        return {
            'served_op_lv_dir': str(self.served_op_lv_dir),
            'SS': self.SS,
            'MUW': self.MUW
        }

    def __jsonrepr__(self):
        return self.__simplejsonrepr__()
...
class CostModelEvaluation:
    ...
    def __init__(self, *, accelerator: 'Accelerator', layer: LayerNode | MemNode, spatial_mapping: 'SpatialMapping', temporal_mapping: 'TemporalMapping'):
        """
    ...
    def run(self):
        ...
        self.calc_data_move_per_mem_instance()
        self.calc_energy()
        self.calc_latency()
    ...
    def calc_memory_utilization(self):
        ...
        self.calc_memory_utilization_per_instance()

    def calc_memory_utilization_per_instance(self):
        mem_utili_instance = {
            str(mem_ins): sum(self.mem_utili_individual[self.mem_op_to_layer_op[memop]][lv] for memop, lv in mem_ins.mem_level_of_operands.items())
            for mem_ins in self.mem_instance_list
        }
        self.mem_utili_instance = mem_utili_instance
    ...
    def calc_memory_word_access(self):
        ...
        for layer_op in self.layer.operand_list:
            ...
            for mem_lv in range(self.mapping.mem_level[layer_op]):
                ...
                ''' All '''
                memory_word_access_sgl = FourWayDataMoving(rd_out_to_low, wr_in_by_low, rd_out_to_high, wr_in_by_high)
                memory_word_access[layer_op].append(memory_word_access_sgl)
    ...
    def calc_energy(self):
        ...
        """
        self.calc_activation_energy_cost()
        self.calc_MAC_energy_cost()
        self.calc_memory_energy_cost()

    def calc_activation_energy_cost(self):
        """ Calculate the activation energy """
        self.activation_energy = 0
        if self.layer.activation_function:
            single_ACT_energy = self.accelerator.get_core(self.core_id).operational_array.unit.cost / 100     # 单次计算激活的能量开销，先随便设一个，假设是单次mac的 1/100
            self.activation_energy = single_ACT_energy * self.layer.operand_size_bit['O']
            if self.layer.activation_function == 'prelu':
                d_src_int = len(self.mem_hierarchy_dict['I2']) -1   # DRAM
                # 针对prelu参数p 的data copy，从DRAM搬到权重的最高片上mem，搬的数据量为输出通道数个（K个）
                self.activation_energy += DataCopyAction(self.layer.loop_dim_size['K'] * 8, ('I2', d_src_int), ('I2', d_src_int-1), self.accelerator.get_core(self.core_id)).energy
    ...
    def calc_memory_energy_cost(self):
        ...
        self.energy_total = self.mem_energy + self.MAC_energy + self.activation_energy
        ...
        for mem_instance in self.mem_instance_list:
            ...
            for port in port_list:
                ...
                for mem_op, mem_lv, mov_dir in port.served_op_lv_dir:
                    ...
                    port_activity_single[str(port)].append(port_activity)   # port_activity_single: Dict[str, List[PortActivity]]
    ...
    def calc_data_loading_offloading_latency(self):
        ...
        if self.layer.activation_function:
            # RELU和PRELU的延迟和并行度有关，比如16*6*6的OFM，如果并行度是16，那就是36个cycle
            parallelism = 16
            experienced_factor = 102400
            self.act_func_latency = ceil(self.layer.operand_size_bit['O'] / (experienced_factor * parallelism))
            self.data_offloading_cycle += self.act_func_latency
    ...
    def calc_overall_latency(self):
        ...
        spatial_stall_cycle = ideal_temporal_cycle - ideal_cycle
        ...
        self.spatial_stall_cycle = spatial_stall_cycle
    ...
    def __add_defines__(self, other):
        ...
        # Not addable
        func = ['calc_allowed_and_real_data_transfer_cycle_per_DTL', 'calc_data_loading_offloading_latency', 'calc_double_buffer_flag', 'calc_extra_datacopy',
                'calc_overall_latency', 'calc_MAC_energy_cost', 'calc_energy', 'calc_latency', 'calc_memory_energy_cost', 'calc_activation_energy_cost',
    ...
    def __add__(self, other):
        ...
        func = [
            ...
            "run",
            "calc_extra_datacopy",
            "calc_activation_energy_cost",
            "calc_data_move_per_mem_instance",
        ]
        add_attr = [
            "MAC_energy",
            "mem_energy",
    ...
    #         logger.info(f"Processing {self.layer.equation} Layer, data amount is {self.layer.operand_size_bit['O']}, data src is {d_src_mem}, and des is {d_des_mem}")
    #         # 两个 data copy action, 从 DRAM搬进来，再搬出去
    #         dca1 = DataCopyAction(data_amount = self.layer.operand_size_bit['O'], data_source = d_src, data_destination = d_des, core = self.accelerator.get_core(self.core_id))
    #         dca2 = DataCopyAction(data_amount = self.layer.operand_size_bit['O'], data_source = d_des, data_destination = d_src, core = self.accelerator.get_core(self.core_id))
    ...
    def calc_data_move_per_mem_instance(self):
        """提取 unit_mem_data_movement 中的 data_elem_move_count, 然后总和起来, 表示数据读写移动量
        """
        raw = self.mapping.unit_mem_data_movement
        demc = {op: [data_dict.data_elem_move_count._info_sum() for data_dict in raw[op]] for op in self.mapping.operand_list}
        self.demc = demc  #data elem move count，按照三个操作数区分和排列
        '''
        demc is like:
        {
            "O": [
        ...
        # mem_ins_demc = {}
        # for mem_ins in self.mem_instance_list:
        #     mem_ins_demc[mem_ins] = 0
        #     mem_lv_of_op = mem_ins.mem_level_of_operands    # e.g. (dram): mem_level_of_operands:  {'I1': 1, 'I2': 2, 'O': 2}
        #     for (memop, lv) in mem_lv_of_op.items():
        #         mem_ins_demc[mem_ins] += demc[self.mem_op_to_layer_op[memop]][lv]
        mem_ins_demc = {
            str(mem_ins): sum(demc[self.mem_op_to_layer_op[memop]][lv] for memop, lv in mem_ins.mem_level_of_operands.items())
            for mem_ins in self.mem_instance_list
        }
        self.mem_ins_demc = mem_ins_demc    # memory instance data elem move count
    ...
    def __simplejsonrepr__(self):
        ...
        return{
            "layer": {
                ...
                "layer loop size": str(self.layer.loop_dim_size),
            },
            "energy(mJ)": (self.energy_total) / 1e9,
            "latency(mC)": (self.latency_total2) / 1e6,
            "MAC utilization" : self.MAC_utilization2,
        }
    ...
    def __jsonrepr__(self):
        ...
        return {
            ...
            "energy": {
                "total_energy(mJ)": (self.energy_total) / 1e9,
                "energy equation": 'total_energy = mem_energy(can be breakdown) + MAC_energy + activation_energy',
                "mem energy": self.mem_energy / 1e9,
                "MAC energy": self.MAC_energy / 1e9,
                "activation_energy": self.activation_energy / 1e9,
                "energy breakdown": {k: [(vv / 1e9) for vv in v] for k, v in self.energy_breakdown.items()},
                # "energy breakdown1": {k: (sum(v) / 1e9) for k, v in self.energy_breakdown.items()},
                # "energy breakdown2": sum({k: sum(v) for k, v in self.energy_breakdown.items()}.values()) / 1e9,
            },
            "MAC utilization": {
                "MAC spatial utilization": self.MAC_spatial_utilization,
                "MAC utilization 0": self.MAC_utilization0,
                "MAC utilization 1": self.MAC_utilization1,
                "MAC utilization 2": self.MAC_utilization2,
            },
            "latency": {
                "latency equation":
                    'total latency(mC) = MAC latency + onloading latecny + offloading latency;  MAC latency = ideal computation cycle + spatial stall + temporal stall; activation latency is merged in offloading latency',
                "total_latency": (self.latency_total2) / 1e6,
                "MAC latency": self.latency_total0 / 1e6,
                "data onloading latency": self.data_loading_cycle / 1e6,
                "data offloading latency": self.data_offloading_cycle / 1e6,
                "ideal computation": self.ideal_cycle / 1e6,
                "spatial stall": self.spatial_stall_cycle / 1e6,
                "temporal stall (SS_comb)": self.SS_comb / 1e6,
                "SS_comb_collect": self.SS_comb_collect,
                # "port_activity_collect": self.port_activity_collect,
                "activation latency": (self.act_func_latency / 1e6 if hasattr(self, 'act_func_latency') else None),
            },
            "mapping": {
                "spatial mapping dict": {op: str(list_) for (op, list_) in self.spatial_mapping.mapping_dict_origin.items()},
                "temporal mapping dict": {op: str(list_) for (op, list_) in self.temporal_mapping.mapping_dic_origin.items()},
                # "temporal mapping sta": {op: str(list_) for (op, list_) in self.temporal_mapping.mapping_dic_stationary.items()},
                "unit_mem_data_movement ": self.mapping.unit_mem_data_movement,
                "spatial mapping unit count": self.mapping.spatial_mapping.unit_count,
            },
            "memory": {
                "mem_data_move_operands": self.demc,
                "mem_data_move_instance": self.mem_ins_demc,
                "double buffer true": self.double_buffer_true,
                "mem_utili_operands": self.mem_utili_shared,
                "mem_utili_instance": self.mem_utili_instance,
                "mem_word_access": self.memory_word_access,
                "activate mem lv": self.active_mem_level,
            },
        }
...
if __name__ == "__main__":
    ...
    cme = CostModelEvaluation(spatial_mapping=spatial_mapping, temporal_mapping=temporal_mapping, accelerator=accelerator, layer=layer_node)

    pprint(cme.demc)
    pprint(cme.mem_ins_demc)
    print()
    pprint(cme.mapping.unit_mem_data_movement)
