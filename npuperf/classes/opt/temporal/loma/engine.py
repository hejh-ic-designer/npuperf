...
import npuperf.classes.io.input_config as inputs
...
from npuperf.classes.opt.temporal.loma.cost_model import CostModelCaller
...
class LomaEngine:
    ...
    def get_temporal_loops(self):
        ...
        for spatial_loop in self.spatial_mapping.spatial_loop_dim_size:
            ...
            q = int(np.ceil(temporal_loop_dim_size.get(spatial_loop_dim, 1) / spatial_loop_size))
            ...
            if q == 1 and temporal_loop_dim_size.get(spatial_loop_dim):
    ...
    def og(self):
        ...
        if self.lpfs == []:
            return [[]]
            # raise ValueError(f'self.lpfs is an empty list. Sorting an empty list may cause errors. Please check')
...
if __name__ == "__main__":
    ...
    # After lpf limit: (set lpf_limit = 6)
