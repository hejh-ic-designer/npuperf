...
class DepthFirstStage_ref(Stage):
    """
    ...
    def run(self):
        ...
        # with open('substack_ME.txt', 'w') as fff:
        #     for i in substacks:
        #         print(i,file=fff)
        # exit()
            ...
            def run_for_tilesize(tile_x, tile_y, weights_from_top,
                ...
                for layer in usefull_workload_tile.topological_sort():
                    ...
                    while True:  # serves more as a goto target here
                        ...
                        #= 例如就像这种情况，X和Y都来自I1： 'memory_operand_links': {'O': 'O', 'X': 'I1', 'Y': 'I1'}
                        if [layer.memory_operand_links[I] for I in layer.input_operands] == ['I1', 'I1']:
                            while memhier.remove_operator_top_level('I2')[0]:   # 把I2 拆光了
                                pass
                            ...
                            # 这是把workload里的 Y 又改成了I2，但是现在mem hier里真正的I2 已经被删光了，这里改成的I2 实际上也是I1，这样做是为了空间映射stage做准备，否则两个都来自I1会报错
                        ...
                        try: # try except to catch edge cases where reserved space turns out not to be enough, in which case we throw stuff at the wall
                            if is_substack_input:
                                ...
                                sub_stage = self.list_of_callables[0](self.list_of_callables[1:], **kwargs_2_ps)
...
#=################################################################### SUB STAGE 入口 ##########################################################################################
        ...
        # 这里向上一级返回结果，抓取 cost_model_evaluations_per_layer 里的信息，然后返回
