...
def print_printing_block(printing_block, fp):
    print(file=fp)
    for i in range(len(printing_block)):
        print("".join(printing_block[i]), file=fp)


def print_good_tm_format(tm, mem_name, cme_name, mem_op_to_layer_op, fp):
    ...
    print_printing_block(tm_block, fp)


        
def print_mapping(cme, fp):
    ...
    cme_name = str(cme)
    print_good_tm_format(tm, mem_name, cme_name, layer_op_to_mem_op, fp)


def print_mapping_tofile(cme_list, file_path):
    """ 
    :param cme_list: List[CME]
    :param file_path: target temporal mapping path to print
    """
    with open(file_path, 'w') as fp:
        for cme in cme_list:
            print_mapping(cme, fp)
